...
func TestGatewayNamespaceMode(t *testing.T) {
	flag.Parse()
	...
	tlog.Logf(t, "Running %d GatewayNamespaceMode tests", len(tests.GatewayNamespaceModeTests))
	err = cSuite.Run(t, tests.GatewayNamespaceModeTests)
	if err != nil {
		t.Fatalf("Failed to run GatewayNamespaceMode tests: %v", err)
	}
}
